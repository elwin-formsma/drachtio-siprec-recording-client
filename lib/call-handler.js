...
module.exports = (req, res) => {
  ...
  logger.info(`received INVITE from ${req.source_address}: uri: ${req.uri}  ${JSON.stringify(uri)}`);

  const srf = req.srf;
  const mrf = new Mrf(srf);
  const opts = { srf, req, logger, sdp: req.body };
  opts.UserToUser = req.get('User-to-User');
  ...
  mrf.connect(config.get('freeswitch'))
    ...
    .then(connectCall.bind(null, opts, req, res, uri.params.dst || req.uri))
    .then(connectSiprec.bind(null, opts))
    .then((opts) => {
      opts.uac.on('destroy', () => {
        logger.info('callee hung up, ending call');
        opts.uas.destroy();
        opts.epIncoming.destroy();
        opts.epOutgoing.destroy();
        opts.epSiprecCaller.destroy();
        opts.epSiprecCallee.destroy();
        cleanup(opts);
      });
      opts.uas.on('destroy', () => {
...
function createConference(opts, arr) {
  opts.logger.info('Creating conference');
